...
import { getPromotionsProducts } from "@/utils/api";
  ...
  useEffect(() => {
    const fetchPromotionsProducts = async () => {
      try {
        ...
        const response = await getPromotionsProducts(currentPage);
        const { results, count, page, page_size, total_pages, has_next, has_previous } = response || {};

        // Transformer les promotions au format attendu par ProductCard
        const formattedProducts = (results || []).map((promotion) => {
          // Construire l'URL de l'image
          let imageUrl = "/img/shop01.svg"; // Image par défaut
          if (promotion.product_image) {
            imageUrl = promotion.product_image.startsWith("http")
              ? promotion.product_image
              : `${process.env.NEXT_PUBLIC_API_URL || "http://192.168.8.101:9000"}${promotion.product_image}`;
          }
          ...
          if (promotion.original_price && promotion.promo_price) {
            const reduction =
              ((parseFloat(promotion.original_price) - parseFloat(promotion.promo_price)) /
                parseFloat(promotion.original_price)) *
              100;
            salePercent = `-${Math.round(reduction)}%`;
          } else if (promotion.discount_percent) {
            salePercent = `-${Math.round(promotion.discount_percent)}%`;
          }

          return {
            id: promotion.product,
            name: promotion.product_name,
            slug: promotion.product_slug,
            price: parseFloat(promotion.promo_price) || 0,
            oldPrice: promotion.original_price
              ? parseFloat(promotion.original_price)
              : null,
            image: imageUrl,
            sale: salePercent,
      ...
      <div className={styles.promotionsContainer}>
        <div className={styles.container}>
          ...
          <div className={styles.promotionsInfo}>
            <p className={styles.promotionsTitle}>
            ...
            <>
              ...
              <div className={styles.productsGridContainer}>
                <div className={styles.resultsCount}>
                  <p>
                    {pagination.count} promotion{pagination.count > 1 ? "s" : ""}{" "}
